...
public final class DecoderFormat {
    ...
    static {
        ...
        FORMATS_FOR_MODE.put(DecoderMode.ONE_D_MODE, ONE_D_FORMATS);
        FORMATS_FOR_MODE.put(DecoderMode.PRODUCT_MODE, PRODUCT_FORMATS);
        FORMATS_FOR_MODE.put(DecoderMode.QR_CODE_MODE, QR_CODE_FORMATS);
        FORMATS_FOR_MODE.put(DecoderMode.DATA_MATRIX_MODE, DATA_MATRIX_FORMATS);
        FORMATS_FOR_MODE.put(DecoderMode.AZTEC_MODE, AZTEC_FORMATS);
        FORMATS_FOR_MODE.put(DecoderMode.PDF417_MODE, PDF417_FORMATS);
    }

    private DecoderFormat() {
    }

    public static Set<BarcodeFormat> parseDecodeFormats(Intent intent) {
        Iterable<String> scanFormats = null;
        CharSequence scanFormatsString = intent.getStringExtra(DecoderMode.FORMATS);
        if (scanFormatsString != null) {
            scanFormats = Arrays.asList(COMMA_PATTERN.split(scanFormatsString));
        }
        return parseDecodeFormats(scanFormats, intent.getStringExtra(DecoderMode.MODE));
    }

    public static Set<BarcodeFormat> parseDecodeFormats(Uri inputUri) {
        List<String> formats = inputUri.getQueryParameters(DecoderMode.FORMATS);
        if (formats != null && formats.size() == 1 && formats.get(0) != null) {
            formats = Arrays.asList(COMMA_PATTERN.split(formats.get(0)));
        }
        return parseDecodeFormats(formats, inputUri.getQueryParameter(DecoderMode.MODE));
    }
